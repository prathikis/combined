...
A step-by-step guide to deploying a voice AI stack (GRS + Claude/Gemini + STT/TTS) entirely within a customer's GCP project, with no data leaving their VPC.
...
- A **GKE cluster** running GRS (orchestration + Pipecat pipeline).
...
│  │   │  (GRS pods)   │         │  192.168.255.230         │ ──▶ Cloud STT
...
This is the single piece of plumbing that gives GRS private access to Vertex AI (Claude/Gemini), Cloud STT, and Cloud TTS.
...
## Phase 3 — Deploy GRS on GKE in the same VPC
...
### 3.2 Install GRS via Helm

```bash
helm install dograh ./charts/dograh -n dograh --create-namespace
```

GRS pods inherit the VPC's DNS, so any call to `aiplatform.googleapis.com`, `speech.googleapis.com`, or `texttospeech.googleapis.com` automatically routes through the PSC endpoint.
...
Run these from a GRS pod inside the cluster:
