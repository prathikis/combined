...
async def create_transport(
    ...
    *,
    ...
    # not GRS's: ``inbound_encoding`` is what we *send into the call*
    # (GRS → Telnyx), and ``outbound_encoding`` is what we *receive out of
    # the call* (Telnyx → GRS).
    serializer = TelnyxFrameSerializer(
        stream_id=stream_id,
        call_control_id=call_control_id,
        api_key=api_key,
        inbound_encoding="PCMU",  # GRS → Telnyx; matches stream_bidirectional_codec
        outbound_encoding=encoding,  # Telnyx → GRS; from media_format.encoding
