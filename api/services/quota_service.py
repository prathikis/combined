"""Quota checking service for GRS credits.
...
LEGACY_QUOTA_EXCEEDED_MESSAGE = (
    ...
    "Please email founders@dograh.com for additional GRS credits "
    "or change providers in Models configurations."
)

BILLING_V2_QUOTA_EXCEEDED_MESSAGE = (
    "You have exhausted your GRS credits. "
...
async def _authorize_hosted_workflow_run_start(
    *,
    ...
    try:
        ...
        return (
            QuotaCheckResult(
                ...
                error_message="Could not verify GRS credits. Please try again.",
            ),
            True,
        )
    ...
    if (
        ...
        logger.warning(
            "Insufficient GRS billing v2 credits for org {}: {:.2f} credits remaining",
            organization_id,
            remaining,
        )
    ...
    logger.info(
        "GRS billing v2 run authorization passed for org {}: {:.2f} credits remaining",
        organization_id,
        remaining,
    )
    ...
    for api_key in dograh_api_keys:
        try:
            ...
            if remaining < MINIMUM_DOGRAH_CREDITS_FOR_CALL:
                logger.warning(
                    f"Insufficient GRS credits for key ...{api_key[-8:]}: "
                    f"${remaining:.2f} remaining"
                )
                return _insufficient_legacy_quota_result()

            logger.info(
                f"GRS quota check passed for key ...{api_key[-8:]}: "
                f"{remaining:.2f} credits remaining"
            )
        except Exception as e:
            logger.error(f"Failed to check quota for GRS key: {str(e)}")
            error_str = str(e)
