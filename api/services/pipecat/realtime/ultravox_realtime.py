"""GRS subclass of pipecat's Ultravox realtime LLM service.

Ultravox is audio-native and realtime, but prompt and tool configuration is
bound to call creation. GRS therefore cannot lean on in-session updates or
Gemini-style session resumption handles. This wrapper adapts Ultravox to the
GRS engine contract by:
...
- reconstructing Ultravox ``initialMessages`` from GRS context when the
  call must be recreated after a node transition
- appending a transient resumptive user nudge to recreated ``initialMessages``
  after tool-result transitions, without mutating GRS's stored context
- handling GRS-only frames such as user mute and idle append prompts
...
class DograhUltravoxOneShotInputParams(OneShotInputParams):
    """GRS-friendly OneShot params with string voice support."""
...
class DograhUltravoxRealtimeLLMService(UltravoxRealtimeLLMService):
    """Ultravox realtime with GRS engine integration quirks."""
    ...
    async def start(self, frame):
        # GRS defers call creation until the engine queues the node opening.
