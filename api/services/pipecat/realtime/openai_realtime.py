"""GRS subclass of pipecat's OpenAI Realtime LLM service.

Layers GRS engine integration quirks onto upstream-pristine
...
- **One-off LLMMessagesAppendFrame handling** for ephemeral realtime prompts
  like user-idle checks, without mutating GRS's local ``LLMContext``.
...
class DograhOpenAIRealtimeLLMService(OpenAIRealtimeLLMService):
    """OpenAI Realtime with GRS engine integration quirks. See module docstring."""

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self._user_is_muted: bool = False
        # GRS pre-populates self._context via the engine before the first
        ...
        # has finished speaking, matching GRS's Gemini Live behavior.
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, TTSSpeakFrame):
            ...
            # lifecycle even when GRS has already pre-populated self._context.
