...
if [[ ! -d "dograh" ]]; then
    ...
    echo -e "${YELLOW}Please run this script from the directory containing your GRS installation.${NC}"
    echo -e "${YELLOW}If you haven't set up GRS yet, run the remote setup first:${NC}"
...
echo -e "${BLUE}[3/7] Stopping GRS services...${NC}"
cd dograh
...
if docker compose --profile remote ps --quiet 2>/dev/null | grep -q .; then
    ...
    echo -e "${GREEN}✓ GRS services stopped${NC}"
else
...
echo ""
echo -e "${BLUE}[7/7] Starting GRS services through validated startup wrapper...${NC}"
./remote_up.sh
