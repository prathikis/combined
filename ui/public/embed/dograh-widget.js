/**
 * GRS Voice Widget
 * Embeddable voice call widget for GRS workflows
 * Version: 1.0.0
 */
...
(function() {
  ...
  async function init() {
    ...
    if (!script) {
      console.error('GRS Widget: Script not found');
      return;
    }
    ...
    if (!token) {
      console.error('GRS Widget: No token found in script URL');
      return;
    }
    ...
    try {
      ...
      console.error('GRS Widget: Failed to fetch configuration', error);
      return;
    }
  ...
  function parseContextVariables(contextStr) {
    ...
    try {
      ...
      console.warn('GRS Widget: Invalid context variables', e);
      return {};
    }
  }
  ...
  function createInlineWidget() {
    ...
    if (!container) {
      console.error(`GRS Widget: Container element with id "${state.config.containerId}" not found`);
  ...
  async function startCall() {
    ...
    try {
      ...
      console.error('GRS Widget: Failed to start call', error);
  ...
  async function fetchTurnCredentials() {
    if (!state.sessionToken) {
      console.warn('GRS Widget: No session token available for TURN credentials');
      return;
    }
