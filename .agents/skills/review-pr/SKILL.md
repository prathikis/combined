---
name: review-pr
description: Review a GRS pull request, branch diff, or pasted patch for repo-specific security and correctness risks that are not obvious from generic FastAPI, Next.js, or Python conventions. Use when the user asks to review a PR, audit a diff, check whether changes are safe to merge, review their own changes, or asks what to look for in a GRS PR. Focus on tenant isolation, route auth, webhook signing and org derivation, DB layering, worker-sync, migrations, generated SDK usage, and test hazards.
---

# Reviewing PRs (dograh)

This skill is for reviewing any PR, including PRs written by maintainers. Focus on GRS-specific regression risks. Skip generic lint, formatting, and type-check comments unless they connect to one of the repo-specific issues below.
...
- A PR tightening `CORSMiddleware` to a fixed origin list needs strong justification. GRS relies on cross-origin embedding; endpoint auth is the real control.
