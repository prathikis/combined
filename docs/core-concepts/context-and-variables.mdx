...
GRS has a simple data model for passing information through a call. Understanding it is key to building agents that feel personalised and to extracting useful results after a call.
...
When the call starts, GRS substitutes the values before sending the prompt to the LLM — so the agent speaks naturally as if it already knows the contact.
...
For telephony calls (inbound and outbound), GRS automatically adds these variables to `initial_context`:
...
For **inbound** calls, `caller_number` is the customer's number and `called_number` is your GRS number. For **outbound** calls, it's the reverse — `caller_number` is your GRS number and `called_number` is the customer's number.
