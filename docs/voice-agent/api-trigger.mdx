...
The API Trigger node lets you initiate outbound calls to your voice agent programmatically. When you add an API Trigger node to your workflow, GRS generates a unique endpoint URL that external systems can call to start a conversation.
...
When you add an API Trigger node to your workflow, GRS assigns it a unique UUID. The trigger node exposes two URLs that share this UUID:
