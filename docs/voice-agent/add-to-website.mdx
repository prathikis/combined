...
description: Add your GRS voice agent to any website so visitors can talk to it.
tag: "NEW"
---
...
<script>
  ...
  window.DograhWidget.onError((err) => {
    console.error('GRS error:', err.message);
  });
...
export function TalkButton() {
  ...
  useEffect(() => {
    ...
    window.DograhWidget.onError((err) => console.error('GRS error:', err.message));
  }, []);
