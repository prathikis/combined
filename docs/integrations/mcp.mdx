---
title: "MCP Server"
description: "Connect Codex, Claude, and other AI assistants to your GRS workspace via the Model Context Protocol"
---

## Overview

GRS exposes an [MCP (Model Context Protocol)](https://modelcontextprotocol.io/) server that lets AI assistants like Codex, Claude Code, Claude Desktop, and Cursor access your workspace and documentation. Once connected, an assistant can list your agents, fetch agent definitions, and search GRS docs on your behalf.

## Prerequisites

- A GRS API key. Generate one at [`/api-keys`](https://app.dograh.com/api-keys) (or `http://localhost:3010/api-keys` for self-hosted). See [API Keys](/configurations/api-keys).
- Your GRS MCP endpoint:
  - Hosted: `https://app.dograh.com/api/v1/mcp/`
  - Self-hosted: `<YOUR_BACKEND_URL>/api/v1/mcp/`

The endpoint is also shown in **Platform Settings → MCP Server** inside the GRS UI.

<Note>
If you deployed GRS to a remote server using [`setup_remote.sh`](/deployment/docker#option-2%3A-remote-server-deployment), your endpoint is served with a self-signed SSL certificate. Claude Code will refuse MCP connections to it unless you start the CLI with TLS verification disabled:
...
Register GRS as an MCP server with the Claude Code CLI:
...
Restart Claude Desktop after saving. The GRS tools should appear in the tool picker.
...
Once the MCP server is connected, you can drive GRS from your coding agent in plain English. A few prompts to try:

**Explore your workspace**

- "List my agents in GRS."
- "Show me the definition of the agent called *Lead Qualifier*."
- "Which credentials and tools are configured in my GRS workspace?"
...
- "Search the GRS docs for how to configure a TURN server."
- "What node types does GRS support, and what fields does a `knowledge_base` node take?"
- "How do I deploy GRS on a custom domain with HTTPS?"

<Note>
Agent edits are saved as a new **draft** version — your published agent keeps serving calls until you explicitly publish the draft from the GRS UI.
</Note>
