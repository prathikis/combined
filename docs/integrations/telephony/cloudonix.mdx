...
description: "Configure Cloudonix for voice communication in GRS AI"
---

## Overview

Cloudonix is a cloud communications platform that enables GRS voice agents to connect to SIP communications
...
- A Cloudonix **Voice Application** on that domain (optional — leave the field blank in GRS and we'll auto-create one for you on save, with the application `url` pre-set)
- A Cloudonix outbound voice trunk service provider connection
- GRS AI instance running and accessible

## Video Tutorial

Watch this step-by-step guide to set up Cloudonix with GRS AI:

<iframe
  className="w-full aspect-video rounded-xl"
  src="https://www.youtube.com/embed/qLKX0F99jpU?si=tA3zwCsNRKe-WCI1"
  title="GRS Cloudonix Setup"
...
3. (Optional) Navigate to your domain's **Applications** and create (or open) the application you'll use with GRS, then copy its **Application Name**. Skip this if you want GRS to auto-create the Voice Application for you on save.

### Step 2: Configure in GRS AI
...
4. Enter your credentials:
   - Bearer Token
   - Domain ID
   - Application Name — *optional*. Leave blank and GRS will auto-create a Voice Application on save (with the application `url` and CXML runtime already configured) and store its name on this configuration.
...
6. Open the configuration you just created and add at least one **phone number** (with country code in E.164 format, e.g. `+1234567890`). The default caller ID is used for outbound calls.
   ...
   <Note>
     If GRS auto-created the Voice Application for you, you still need to
...
Cloudonix routes inbound calls per **Voice Application** — the webhook URL is set once on the application, and applies to every DNID bound to it. **When you save an inbound workflow on a phone number, GRS automatically pushes the webhook URL to your Voice Application's `url`** (provided the credentials are correct), so you don't need to set the webhook by hand. If GRS auto-created the application during configuration save, the `url` is already set and this push is a no-op — you only need to bind your DNIDs to the auto-created application.
...
2. Create a new application (or open the existing one whose name you configured in GRS) with these settings:
...
4. Save
   ...
   <Note>
     The Application URL is what GRS's auto-push updates in Step 3 — you
     ...
     Either works. Self-hosted GRS deployments use their backend domain
   ...
   <img
     ...
     alt="Cloudonix application form showing Remote Application Resource type, Cloudonix CXML runtime, the GRS inbound URL, POST method, and a DNID number entry"
   />
   <img
     className="hidden dark:block"
     src="/images/cloudonix-inbound-config-application.png"
...
### Step 3: Assign an Inbound Workflow to the Phone Number in GRS
...
2. Open the application whose name you configured in GRS
3. Confirm:
   - **URL** is set to: `https://api.dograh.com/api/v1/telephony/inbound/run`
   - **Method** is `POST`

   <Note>
     GRS pushed this URL automatically when you saved the inbound workflow
     in Step 3. The same URL is shared across every DNID bound to that
     application — GRS routes each inbound call to the right agent based
     on the called number's inbound workflow assignment. If the field is
     empty, shows a different URL, or GRS surfaced a sync warning on
     save, the auto-push failed — most often because the Bearer Token,
     Domain ID, or Application Name in GRS is incorrect. Paste the URL
     into the field yourself, set the method to `POST`, and save. On
     self-hosted GRS, replace `api.dograh.com` with your backend domain.
   </Note>
...
- Ensure your GRS AI instance is publicly accessible
...
2. Verify your GRS AI voice agent answers and responds
3. Check call logs in both GRS AI dashboard and Cloudonix Console
...
<AccordionGroup>
  ...
  <Accordion title="Inbound calls not reaching voice agent">
    ...
    configured in GRS - Confirm the called number exists in your GRS
    telephony configuration and has an **Inbound workflow** assigned -
    After assigning the inbound workflow, confirm GRS successfully updated
  ...
  <Accordion title="Voice agent doesn't respond to inbound calls">
    ...
    /telephony-configurations - Verify the Bearer Token in GRS matches the
...
- Use a dedicated Voice Application for GRS so the shared `url` doesn't conflict with other systems
