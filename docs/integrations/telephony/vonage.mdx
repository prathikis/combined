...
description: "Configure Vonage (Nexmo) for voice communication in GRS AI"
---

## Overview

Vonage (formerly Nexmo) is a cloud communications platform that provides global voice, messaging, and video capabilities. GRS AI's Vonage integration enables high-quality voice interactions with your agents using Vonage's robust infrastructure.
...
Vonage configures inbound webhooks at the **application level**, not per phone number. A single **Answer URL** on the Vonage application applies to every number linked to it. GRS routes the call to the right agent based on the called number's inbound workflow assignment inside GRS. **When you save an inbound workflow on a phone number, GRS automatically pushes the webhook URL to your Vonage Application's Answer URL** (provided the credentials are correct).

### Step 1: Link Phone Numbers to Your Vonage Application

1. Open the [Vonage Dashboard](https://dashboard.nexmo.com/)
2. Under **Numbers** → **Your Numbers**, link each number you want to use for inbound to the same Vonage Application whose ID you configured in GRS
...
2. Under **Capabilities** → **Voice**, confirm:
   ...
   <Note>
     ...
     in Step 2. If the field is empty, shows a different URL, or GRS
     surfaced a sync warning on save, the auto-push failed — most often
     because the API Key/Secret or Application ID in GRS is incorrect.
     Paste the URL into the field yourself, set the method to `POST`, and
     save the application. On self-hosted GRS, replace `api.dograh.com`
...
3. Check call logs in both GRS AI dashboard and Vonage Dashboard
...
<AccordionGroup>
  ...
  <Accordion title="Inbound calls not reaching voice agent">
    ...
    - Confirm the called number exists in your GRS telephony configuration and has an **Inbound workflow** assigned
  </Accordion>
  ...
  <Accordion title="Voice agent doesn't respond to inbound calls">
    ...
    - Verify API Key matches the one stored in your GRS telephony configuration (used to identify the org from the inbound webhook)
