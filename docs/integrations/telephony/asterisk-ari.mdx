...
description: "Connect GRS AI to your Asterisk PBX using the Asterisk REST Interface (ARI)"
---

## Overview

Asterisk ARI (Asterisk REST Interface) allows you to connect GRS AI voice agents to your existing Asterisk PBX. ARI provides a WebSocket-based event model for controlling calls via Stasis applications, giving GRS full control over call flow and audio streaming.

This guide focuses on the GRS-specific configuration. For general Asterisk installation and administration, refer to the [official Asterisk documentation](https://docs.asterisk.org/).
...
- Network connectivity between your GRS instance and Asterisk
- GRS AI instance running and accessible

<Note>
If you compiled Asterisk from source, ensure both `chan_websocket` and `res_websocket_client` are included during the build. These modules are required for external media streaming between Asterisk and GRS. Refer to the [Asterisk build system documentation](https://docs.asterisk.org/) for details on enabling modules.
</Note>

## Asterisk Configuration

The following Asterisk configuration files need to be set up to work with GRS. These are minimal examples focused on the GRS integration -- refer to the [Asterisk documentation](https://docs.asterisk.org/) for full configuration details.

### Enable ARI (`ari.conf`)

Create an ARI user that GRS will use to authenticate:
...
<Note>
The username (section name, e.g., `dograh`) and password here must match the **Stasis App Name** and **App Password** you configure in GRS.
</Note>
...
Route incoming calls to your Stasis application so GRS can handle them:
...
Replace `dograh` with the app name you configured in `ari.conf` and in GRS.

### Configure External Media Streaming (`websocket_client.conf`)

GRS uses Asterisk's external media streaming to send and receive audio over WebSocket. Configure a WebSocket client connection that points to your GRS instance:
...
<Tabs>
  <Tab title="GRS Cloud">
    ...
    <Note>
    `tls_enabled = yes` is required even though the URI scheme is `wss://` — without it Asterisk will not negotiate TLS and the connection will fail. The ARI credentials (**Stasis App Name** and **App Password**) must match what you configure in the GRS dashboard under Telephony Settings.
    </Note>
  </Tab>
  <Tab title="Self-hosted">
    ...
    <Note>
    Self-hosted deployments on an internal network may use an unencrypted WebSocket (`ws://`). If your GRS instance is exposed over HTTPS, use `wss://` and the corresponding hostname instead.
    </Note>
  </Tab>
</Tabs>

<Note>
The section name (e.g., `dograh`) is the **WebSocket Client Name** you'll enter in the GRS telephony configuration. This name tells Asterisk which WebSocket connection to use for external media streaming during calls.
</Note>

<Note>
GRS's external media channel uses **G.711 μ-law (`ulaw`)**. Make sure any PJSIP endpoint or SIP trunk that places or receives calls through GRS allows `ulaw` (e.g. `allow=ulaw` in the endpoint config).
</Note>
...
Unlike other telephony providers that use HTTP webhooks for inbound calls, ARI delivers inbound calls as **StasisStart events on the ARI WebSocket**. GRS automatically detects these events and activates the workflow assigned to the called extension.
...
3. GRS looks up the called extension in your telephony configuration's phone numbers, finds the assigned workflow, validates quota, and creates a workflow run
...
…or use a pattern that catches every extension you'll register in GRS:
...
**Step 2: Add the extension as a phone number in GRS**
...
4. Save
   ...
   <Note>
   Adding the extension in GRS doesn't change Asterisk's dialplan — that's
   what Step 1 is for. The GRS entry tells the StasisStart handler which
...
<AccordionGroup>
  <Accordion title="Cannot connect to ARI endpoint">
    - Verify the ARI endpoint URL is correct and reachable from your GRS instance
  ...
  <Accordion title="No audio during calls">
    ...
    - Check that `websocket_client.conf` is correctly configured with the right GRS URI
    - Ensure the WebSocket Client Name in GRS matches the section name in `websocket_client.conf`
    - Verify network connectivity and firewall rules allow WebSocket traffic between Asterisk and GRS
  </Accordion>

  <Accordion title="Calls not reaching GRS">
  ...
  <Accordion title="Inbound calls are immediately hung up">
    ...
    - Review GRS logs for warnings like "no matching phone number registered
    for config" or "has no inbound_workflow_id assigned"
  </Accordion>

  <Accordion title="WebSocket client connection issues">
    - Check the URI in `websocket_client.conf` points to the correct GRS host and port
    - Verify the GRS instance is running and accepting WebSocket connections
...
- Keep your Asterisk instance on the same network or a low-latency connection to GRS for optimal audio quality
- Use strong passwords for ARI authentication
- Restrict ARI access to known IP addresses using firewall rules
- Monitor Asterisk logs alongside GRS logs when debugging call issues
