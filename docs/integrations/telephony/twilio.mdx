...
description: "Configure Twilio for voice communication in GRS AI"
---

## Overview

Twilio is a cloud communications platform that enables voice calling, messaging, and video capabilities. GRS AI's Twilio integration provides seamless connectivity for your voice agents.
...
Inbound routing is driven by the phone number itself — there is a **single webhook URL** for the entire org, and GRS resolves which agent to run from the called number's assigned inbound workflow. **When you save an inbound workflow on a phone number, GRS automatically pushes the webhook URL to that number's `VoiceUrl` in your Twilio account** (provided the credentials are correct and the number belongs to that account).
...
3. In the **Voice Configuration** section, confirm:
   ...
   <Note>
     ...
     in Step 1. If the field is empty, shows a different URL, or GRS
     ...
     `POST`, and click **Save Configuration**. On self-hosted GRS,
...
3. Check call logs in both GRS AI dashboard and Twilio Console
...
<AccordionGroup>
  ...
  <Accordion title="Inbound calls go to voicemail">
    ...
    exists in your GRS telephony configuration and has an **Inbound
    workflow** assigned - Verify GRS AI instance is running and responding
  </Accordion>
  ...
  <Accordion title="Voice agent doesn't respond to inbound calls">
    ...
    (Auth Token in GRS matches Twilio Console) - Verify WebSocket connection
