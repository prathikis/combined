...
description: "Complete reference for all environment variables used by the GRS backend"
---
...
GRS supports two deployment modes, set via `DEPLOYMENT_MODE`:

- **OSS**: The default mode. Designed for self-hosted deployments using [Docker Compose](/deployment/docker) — the fastest way to get GRS running. Uses local JWT authentication and MinIO for storage.
- **SaaS**: Intended for customised deployments outside of Docker. Authentication and API key management are handled through GRS Managed Platform Services (MPS), allowing greater flexibility in how the platform is hosted and integrated.
...
| `MPS_API_URL` | `https://services.dograh.com` | GRS Managed Platform Services URL |
...
GRS uses **MinIO by default**, which is bundled with the self-hosted deployment and requires no external setup. Set `ENABLE_AWS_S3=true` to switch to AWS S3 — typically used for cloud or managed deployments where S3 is already part of the infrastructure.
