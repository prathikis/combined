...
description: "Voice Agents need AI Models to work, like LLM (Large Language Model), TTS (Voice) and STT (Transcriber). You can use any of your faviourite providers with GRS Platform to run your Voice Agent."
---

## How Model Configuration Works

GRS uses a **two-level configuration system** for AI models:
...
The global configuration is the default model setup shared across all your agents. GRS ships with its own models by default — when you sign up on https://app.dograh.com or set up the platform on your self-hosted infrastructure, you get some GRS model credits to start with.
...
Select a provider from the dropdown and configure the API key, model, and any provider-specific settings. For GRS's own models, see [Service Keys](api-keys) for instructions on creating Service Keys.
...
GRS supports Gemini 3.1 Live as a **Realtime** provider. The default model is `gemini-3.1-flash-live-preview`.
...
To use Gemini 3.1 Live with GRS, you need a Google Gemini API key. Follow these steps:
...
<Note>
  The Gemini API key is different from a Google Cloud service account key. You specifically need a **Gemini API key** from Google AI Studio for use with GRS.
</Note>

### Configuring Gemini 3.1 Live in GRS

1. Go to **Model Configurations** in your GRS dashboard (`https://app.dograh.com/model-configurations` for hosted or `http://localhost:3010/model-configurations` for local).
...
If you want to run Gemini Live through your own Google Cloud project — for billing consolidation, VPC controls, regional residency, or enterprise IAM — GRS also supports Gemini Live via **Vertex AI** as a separate provider (`google_vertex_realtime`). The default model is `google/gemini-live-2.5-flash-native-audio`.
...
<Note>
  Always pick **JSON**, not P12. The Vertex AI client libraries used by GRS only accept service-account JSON keys; P12 is a legacy format retained for older Google Workspace integrations.
</Note>

### Configuring Vertex AI Realtime in GRS

1. Go to **Model Configurations** in your GRS dashboard.
...
<Note>
  Paste the whole JSON file into the **Credentials** field — including `private_key`, `client_email`, and all other entries. Don't try to extract individual fields. If `Credentials` is left blank, GRS falls back to **Application Default Credentials (ADC)** from the host environment, which is useful when running GRS on a GCP VM or GKE pod with an attached service account.
</Note>
