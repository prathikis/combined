---
title: "Update"
description: "Update your self-hosted GRS stack to a newer image version"
---

This guide covers updating a GRS stack you've already deployed with [Docker](/deployment/docker) or a [custom domain](/deployment/custom-domain). You run commands from the same directory that contains your `docker-compose.yaml` (this is the `dograh/` directory if you used `setup_remote.sh`).
...
GRS publishes two images — `dograh-api` and `dograh-ui` — to both container registries:
